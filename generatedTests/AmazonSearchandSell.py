...
def main():
    ...
    elem = find_elem({'tag': 'input', 'value': '', 'attrs': {'type': 'search', 'placeholder': 'Search Amazon'}})
    
    elem.send_keys("RTX 4090\n")
    
    time.sleep(3)
    send_screenshot(sio)

    elem = find_elem({'tag': 'a', 'value': 'Sell', 'attrs': {'name': 'sell', 'value': 'sell', 'href': ''}})
