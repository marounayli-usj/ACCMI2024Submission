...
def main():
    ...
    elem = find_elem({'tag': 'button', 'value': 'Login', 'attrs': {'id': 'login-button', 'class': 'top-button', 'style': 'position: absolute; top: 0;', 'onclick': 'login()'}})
    ...
    elem = find_elem({'tag': 'input', 'value': '', 'attrs': {'type': 'text', 'name': 'username_or_email', 'value': ''}})
    
    elem.send_keys("marounayle@gmail.com")
    
    time.sleep(3)
    send_screenshot(sio)

    elem = find_elem({'tag': 'input', 'value': '', 'attrs': {'type': 'password'}})
    
    elem.send_keys("pepehandsmonkas")
